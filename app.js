// DYSTORE v4 — Original Quality / Safe 60 FPS
...
function choose(file) {
  ...
  inspect(file);
  $("#outType").textContent = "Original quality · source FPS preserved";
...
async function fastPatch() {
  // SAFE 60 FPS MODE:
  // Do NOT inflate sample tables. The previous 10x inflation changed the
  // timing/sample structure and could make a 60fps source appear as 600fps.
  // We only normalize the MP4 container; video/audio samples are untouched.
  setProgress(12, "Loading safe MP4 patch…");

  const { normalizeContainer } = await import(CDN + "mp4-normalize.mjs");

  setProgress(30, "Reading video locally…");
  const buffer = await selected.arrayBuffer();
  const bytes = new Uint8Array(buffer);
  const view = new DataView(buffer);

  setProgress(55, "Normalizing MP4 container…");
  const normalized = normalizeContainer(bytes, view);
  if (!normalized?.valid) throw new Error("Invalid or unsupported MP4 container.");

  setProgress(85, "Preserving original video samples…");
  // IMPORTANT: no sample-table inflation and no re-encoding.
  return new Blob([normalized.newBuffer], { type: "video/mp4" });
}
...
async function processVideo() {
  ...
  try {
    ...
    $("#outType").textContent = "Original quality · Safe 60 FPS";
